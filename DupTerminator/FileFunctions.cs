...
namespace DupTerminator
{
    class FileFunctions
    {
        ...
        public SettingsApp settings; //= new SettingsApp(); //珏祜??觌囫襦 ?磬耱痤殛囔?
        public DBManager dbManager;
        #endregion //"Declarations"

        #region "Events"
        public delegate void FolderChangedDelegate(int i, string folder);  //镟疣戾蝠?蝈 驽 黩??汶噔眍?趔黻鲨?
        public event FolderChangedDelegate FolderChangedEvent;  //磬漕 恹玮囹?镳?耢屙?滂疱牝铕梃
        ...
        private void AddFiles(System.IO.DirectoryInfo di, ref ArrayList al)
        {
            try
            {
                ...
                if (_excludePattern.Count > 0)
                {
                    ...
                    if (nofileZ.Count != 0)
                    {
                        //List<int> toRemove = new List<int>(); //桧溴犟 屐屙蝾?潆 箐嚯屙?
        ...
        private ArrayList ParseBySize(ArrayList inputList)
        {
            ...
            for (int i = 1; i < inputList.Count; i++)
            {
                ...
                {
                    fileSizeMatchList.Add(inputList[i]);  //耦溴疰栩 羿殡??钿桧嚓钼 疣珈屦铎
                }
                else
                {
            ...
            foreach (ArrayList ssg in fileList)
            {
                ...
                if (ssg.Count > 1)
                {
                    ...
                    foreach (ArrayList csg in ChecksumSortedGroup)
                    {
                        ...
                        {
                            //_groupCount++;
                            //_DuplicateCount += csg.Count; //猁耱痤 耠桫觐?
